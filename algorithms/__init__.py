ALGORITHM_LIST = [
    ("Greedy Search", "greedy"),
    ("Greedy Search(Double Moves)", "greedy_ANDOR"),

    ("A* Search", "a_star"),
    ("A* Search (Double Moves)", "a_star_ANDOR"),

    ("BFS", "bfs"),
    ("BFS (Double Moves)", "bfs_ANDOR"),

    ("UCS", "ucs"),
    ("UCS (Double Moves)", "ucs_ANDOR"),

    ("Hill Climbing", "hill_climbing"),
    ("Hill Climbing (Double)", "hill_climbing_ANDOR"),

    ("Stochastic Hill Climbing", "stochastic_hill"), # Corrected from stochastic_hc
    ("Stochastic Hill Climbing (Double)", "stochastic_hill_ANDOR"), # Corrected from stochastic_hc_ANDOR

    ("DFS", "dfs"),
    ("DFS (Double Moves)", "dfs_ANDOR"),

    ("Steepest Hill Climbing", "steepest_hill"), # Corrected from steepest_hc
    ("Steepest Hill (Double)", "steepest_hill_ANDOR"), # Corrected from steepest_hc_ANDOR

    ("IDDFS", "iddfs"),
    ("IDDFS (Double Moves)", "iddfs_ANDOR"),

    ("IDA* Search", "ida_star"),
    ("IDA* (Double Moves)", "ida_star_ANDOR"),

    ("Beam Search", "beam_search"),
    ("Beam Search(Double Moves)", "beam_search_ANDOR"),

    ("Simulated Annealing", "simulated_annealing"),
    ("Simulated Annealing (Double)", "simulated_annealing_ANDOR"),
     ("QLearning", "q_learning"),
]