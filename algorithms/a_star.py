...
def solve(start_state, goal_state):
    ...
    while pq:
        visited_count += 1
        f, g, current_state = heappop(pq)
        if current_state == goal_state:
            return reconstruct_path(current_state, parent), visited_count
        for next_state in get_neighbors(current_state):
            new_g = g + 1
            if next_state not in g_value or new_g < g_value[next_state]:
                g_value[next_state] = new_g
                parent[next_state] = current_state
                heappush(pq, (new_g + manhattan_distance(next_state, goal_state), new_g, next_state))
